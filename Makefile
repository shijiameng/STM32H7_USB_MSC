...
CFLAGS = --target=arm-none-eabi --sysroot=$(TOOLCHAIN_DIR)/arm-none-eabi -g -O0 -std=gnu11 -march=armv7e-m -mcpu=cortex-m4 -DDEBUG -DUSE_HAL_DRIVER -DSTM32H7B3xxQ -DUSE_USB_HS_IN_FS -mfpu=fpv5-d16 -mfloat-abi=hard -mthumb -ffunction-sections -fdata-sections -fshort-enums
LLVM = -flegacy-pass-manager -Xclang -load -Xclang $(MCU_SANITIZER_WORKDIR)/llvmpass/build/uSan/usan.so

LIBS = -lc -lm -lmcuasan-rt
LDFLAGS = -mcpu=cortex-m4 -T./STM32CubeIDE/STM32H7B3LIHXQ_FLASH.ld --specs=nosys.specs -Wl,--gc-sections -static --specs=nano.specs -mfpu=fpv5-d16 -mfloat-abi=hard -mthumb -L$(MCU_SANITIZER_WORKDIR)/compiler-rt -Wl,--start-group $(LIBS) -Wl,--end-group

CORE_C_SRC = $(wildcard Core/Src/*.c)
CORE_C_OBJ = $(CORE_C_SRC:.c=.o)

DRIVERS_C_SRC += $(wildcard Drivers/STM32H7xx_HAL_Driver/Src/*.c)
...
$(TARGET): $(CORE_C_OBJ) $(DRIVERS_C_OBJ) $(MIDDLEWARES_C_OBJ) $(USB_HOST_C_OBJ) $(STARTUP_AS_OBJ)
